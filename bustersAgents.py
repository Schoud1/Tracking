...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        ghostPosList = []
        closestDist = float("inf") #how close is the closest ghost

        for ghostDistribution in livingGhostPositionDistributions: #take all distributions and finds where the ghost is most likely to be found
            mostLikelyPos = ghostDistribution.argMax()
            ghostPosList.append(mostLikelyPos)

            dist = self.distancer.getDistance(mostLikelyPos, pacmanPosition) #checks how far we are from the ghost
            if dist < closestDist:
                closestDist = dist

        for action in legal: #where is pacman after he takes an action
            newPacPos = Actions.getSuccessor(pacmanPosition, action)

            for ghostPos in ghostPosList:
                if self.distancer.getDistance(newPacPos, ghostPos) < closestDist:
                    return action
