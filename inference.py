...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.beliefs = util.Counter()
            self.beliefs[self.getJailPosition()] = 1 #only if ghost is in jail
            return
        ...
        for p in self.legalPositions:
            ...
            allPossible[p] = self.beliefs[p] * emissionModel[trueDistance] #allPossible[p] = P(ghost is at p) * P(reading gives us this distance)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        beliefs = util.Counter() #Make a new counter and use what we know

        for pos in self.legalPositions:  # all the possible positions that we have
            posDistribution = self.getPositionDistribution(self.setGhostPosition(gameState,pos))  # given that a ghost is at a certain position, posDistribution gives the possible locations during next turn and associated probabilities

            for newPos, prob in posDistribution.items():
                beliefs[newPos] += self.beliefs[pos] * prob
                # P(ghost @ newPos) = P(ghost @ oldPos) * P(ghost moved to newPos)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [] #list of samples we have

        for position in self.legalPositions:
            if len(self.particles) == self.numParticles:
                break

            self.particles.append(position) #add a new particle
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.legalPositions:
            posDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, pos))

            for newPos, prob in posDistribution.items():
                newBeliefs[newPos] += currBeliefs[pos] * prob

        newBeliefs.normalize() #probabilities add up to one
